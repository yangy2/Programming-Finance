...
void polynomial::getdata()
{
	...
	for (int i = 0; i < terms; i++)
	{
		...
		do
		{
			...
			cin >> coeff >> exp;
			if (cin.fail() || exp < 0 || exp > 6) //Error checking if input is not valid
			{
				cout << "\t\tCoefficient and Exponent must be integers" << endl;
				cout << "\t\tExponent must be between 0 and 6 (inclusive)" << endl;
			}
			refresh_cin();
		} while (cin.fail() || exp < 0 || exp > 6); //Repeat if input was invalid
...
polynomial polynomial::operator-=(polynomial poly)
{
	...
	for (int i = 0; i < size; i++)
	{
		result.P[i] = P[i] - poly.P[i];   //Subtracting  individual elements of the Polynomial
	}
	return result;
}
